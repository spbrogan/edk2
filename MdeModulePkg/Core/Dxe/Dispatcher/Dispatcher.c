/** @file
  ...
  Step #1 - When a FV protocol is added to the system every driver in the FV
            is added to the mDiscoveredList. The SOR, Before, and After Depex
are pre-processed as drivers are added to the mDiscoveredList. If an Apriori
...
//
LIST_ENTRY mFvHandleList =
    INITIALIZE_LIST_HEAD_VARIABLE(mFvHandleList); // list of KNOWN_HANDLE
...
**/
VOID CoreInsertOnScheduledQueueWhileProcessingBeforeAndAfter(
...
**/
VOID EFIAPI CoreFwVolEventProtocolNotify(IN EFI_EVENT Event, IN VOID *Context);
...
/**
  ...
  @param  FvHandle              Handle for Fv, needed in the
                                EFI_CORE_DRIVER_ENTRY so that the PE image can
be read out of the FV at a later time.
...
CoreFvToDevicePath(IN EFI_FIRMWARE_VOLUME2_PROTOCOL *Fv, IN EFI_HANDLE FvHandle,
...
be read out of the FV at a later time.
  ...
  @retval EFI_ALREADY_STARTED   The driver has already been started. Only one
                                DriverName may be active in the system at any
one time.

**/
EFI_STATUS
CoreAddToDriverList(IN EFI_FIRMWARE_VOLUME2_PROTOCOL *Fv,
                    IN EFI_HANDLE FvHandle, IN EFI_GUID *DriverName,
...
every Fv image(s).
  ...
  @param  Fv                    The FIRMWARE_VOLUME protocol installed on the
FV.
...
**/
EFI_STATUS
CoreProcessFvImageFile(IN EFI_FIRMWARE_VOLUME2_PROTOCOL *Fv,
                       IN EFI_HANDLE FvHandle, IN EFI_GUID *FileName);

/**
  Enter critical section by gaining lock on mDispatcherLock.

**/
VOID CoreAcquireDispatcherLock(VOID) { CoreAcquireLock(&mDispatcherLock); }

/**
  Exit critical section by releasing lock on mDispatcherLock.

**/
VOID CoreReleaseDispatcherLock(VOID) { CoreReleaseLock(&mDispatcherLock); }

/**
  Read Depex and pre-process the Depex for Before and After. If Section
Extraction protocol returns an error via ReadSection defer the reading of the
Depex.

  @param  DriverEntry           Driver to work on.

  @retval EFI_SUCCESS           Depex read and preprossesed
  @retval EFI_PROTOCOL_ERROR    The section extraction protocol returned an
error and  Depex reading needs to be retried.
  @retval Error                 DEPEX not found.

**/
EFI_STATUS
CoreGetDepexSectionAndPreProccess(IN EFI_CORE_DRIVER_ENTRY *DriverEntry) {
  EFI_STATUS Status;
  ...
  Status =
      Fv->ReadSection(DriverEntry->Fv, &DriverEntry->FileName, SectionType, 0,
                      &DriverEntry->Depex, (UINTN *)&DriverEntry->DepexSize,
...
/**
  Check every driver and locate a matching one. If the driver is found, the
Unrequested state flag is cleared.
...
EFI_STATUS
EFIAPI
CoreSchedule(IN EFI_HANDLE FirmwareVolumeHandle, IN EFI_GUID *DriverName) {
  LIST_ENTRY *Link;
  ...
  // Check every driver
  //
  for (Link = mDiscoveredList.ForwardLink; Link != &mDiscoveredList;
       Link = Link->ForwardLink) {
    DriverEntry =
        CR(Link, EFI_CORE_DRIVER_ENTRY, Link, EFI_CORE_DRIVER_ENTRY_SIGNATURE);
...
EFI_STATUS
EFIAPI
CoreTrust(IN EFI_HANDLE FirmwareVolumeHandle, IN EFI_GUID *DriverName) {
  LIST_ENTRY *Link;
  EFI_CORE_DRIVER_ENTRY *DriverEntry;
...
CoreDispatcher(VOID) {
  ...
  Status =
      CoreCreateEventEx(EVT_NOTIFY_SIGNAL, TPL_NOTIFY, EfiEventEmptyFunction,
  ...
  do {
    ...
    while (!IsListEmpty(&mScheduledQueue)) {
      DriverEntry = CR(mScheduledQueue.ForwardLink, EFI_CORE_DRIVER_ENTRY,
                       ScheduledLink, EFI_CORE_DRIVER_ENTRY_SIGNATURE);

      //
      // Load the DXE Driver image into memory. If the Driver was transitioned
      // from Untrused to Scheduled it would have already been loaded so we may
      // need to skip the LoadImage
      //
      if (DriverEntry->ImageHandle == NULL && !DriverEntry->IsFvImage) {
        DEBUG((DEBUG_INFO, "Loading driver %g\n", &DriverEntry->FileName));
        Status = CoreLoadImage(FALSE, gDxeCoreImageHandle,
                               DriverEntry->FvFileDevicePath, NULL, 0,
        ...
        if (EFI_ERROR(Status)) {
          ...
          if (Status == EFI_SECURITY_VIOLATION) {
            ...
            //
            // The DXE Driver could not be loaded, and do not attempt to load or
            // start it again. Take driver from Scheduled to Initialized.
            //
            // This case include the Never Trusted state if EFI_ACCESS_DENIED is
            // returned
            //
      ...
      if (DriverEntry->IsFvImage) {
        ...
        // dispatched from.
        //
        Status = CoreProcessFvImageFile(DriverEntry->Fv, DriverEntry->FvHandle,
                                        &DriverEntry->FileName);
      } else {
        REPORT_STATUS_CODE_WITH_EXTENDED_DATA(
            EFI_PROGRESS_CODE, (EFI_SOFTWARE_DXE_CORE | EFI_SW_PC_INIT_BEGIN),
            &DriverEntry->ImageHandle, sizeof(DriverEntry->ImageHandle));
    ...
    for (Link = mDiscoveredList.ForwardLink; Link != &mDiscoveredList;
         Link = Link->ForwardLink) {
      DriverEntry = CR(Link, EFI_CORE_DRIVER_ENTRY, Link,
...
VOID CoreInsertOnScheduledQueueWhileProcessingBeforeAndAfter(
  ...
  for (Link = mDiscoveredList.ForwardLink; Link != &mDiscoveredList;
    ...
    if (DriverEntry->Before && DriverEntry->Dependent &&
        DriverEntry != InsertedDriverEntry) {
      DEBUG((DEBUG_DISPATCH, "Evaluate DXE DEPEX for FFS(%g)\n",
             &DriverEntry->FileName));
      DEBUG((DEBUG_DISPATCH,
             "  BEFORE FFS(%g) = ", &DriverEntry->BeforeAfterGuid));
      if (CompareGuid(&InsertedDriverEntry->FileName,
  ...
  for (Link = mDiscoveredList.ForwardLink; Link != &mDiscoveredList;
    ...
    if (DriverEntry->After && DriverEntry->Dependent &&
      ...
      DEBUG((DEBUG_DISPATCH, "Evaluate DXE DEPEX for FFS(%g)\n",
             &DriverEntry->FileName));
      DEBUG((DEBUG_DISPATCH,
             "  AFTER FFS(%g) = ", &DriverEntry->BeforeAfterGuid));
      if (CompareGuid(&InsertedDriverEntry->FileName,
                      &DriverEntry->BeforeAfterGuid)) {
        //
...
**/
BOOLEAN
FvHasBeenProcessed(IN EFI_HANDLE FvHandle) {
  LIST_ENTRY *Link;
  KNOWN_HANDLE *KnownHandle;

  for (Link = mFvHandleList.ForwardLink; Link != &mFvHandleList;
       Link = Link->ForwardLink) {
...
/**
  ...
  @return A point to new added FvHandle entry. If FvHandle with the same FvImage
guid has been added, NULL will return.

**/
KNOWN_HANDLE *FvIsBeingProcessed(IN EFI_HANDLE FvHandle) {
  EFI_STATUS Status;
  EFI_GUID FvNameGuid;
  ...
  Status = CoreHandleProtocol(FvHandle, &gEfiFirmwareVolumeBlockProtocolGuid,
                              (VOID **)&Fvb);
  if (!EFI_ERROR(Status)) {
    //
    ...
    if (!EFI_ERROR(Status)) {
      ...
      if (VerifyFvHeaderChecksum(FwVolHeader) &&
        ...
        LbaOffset = 0;
        //
        // Find LbaIndex and LbaOffset for FV extension header based on
        // BlockMap.
        //
        while ((BlockMap->NumBlocks != 0) || (BlockMap->Length != 0)) {
          for (Index = 0; Index < BlockMap->NumBlocks &&
        ...
        //
        Status = ReadFvbData(Fvb, &LbaIndex, &LbaOffset, sizeof(FvNameGuid),
                             (UINT8 *)&FvNameGuid);
        if (!EFI_ERROR(Status)) {
...
CoreFvToDevicePath(IN EFI_FIRMWARE_VOLUME2_PROTOCOL *Fv, IN EFI_HANDLE FvHandle,
  ...
  //
  Status = CoreHandleProtocol(FvHandle, &gEfiDevicePathProtocolGuid,
                              (VOID **)&FvDevicePath);
  if (EFI_ERROR(Status)) {
    ...
    FileNameDevicePath = AppendDevicePath(
        FvDevicePath, (EFI_DEVICE_PATH_PROTOCOL *)&mFvDevicePath);
  }
...
CoreAddToDriverList(IN EFI_FIRMWARE_VOLUME2_PROTOCOL *Fv,
  ...
  //
  // Create the Driver Entry for the list. ZeroPool initializes lots of
...
**/
BOOLEAN
FvFoundInHobFv2(IN CONST EFI_GUID *FvNameGuid, IN CONST EFI_GUID *DriverName) {
...
**/
BOOLEAN
GetFvUsedSize(IN EFI_FIRMWARE_VOLUME_HEADER *FvHeader, OUT UINT32 *FvUsedSize,
              OUT UINT8 *EraseByte) {
  ...
  if (ExtHeaderOffset != 0) {
    ExtHeader =
        (EFI_FIRMWARE_VOLUME_EXT_HEADER *)((UINT8 *)FvHeader + ExtHeaderOffset);
    ExtEntryList = (EFI_FIRMWARE_VOLUME_EXT_ENTRY *)(ExtHeader + 1);
    while ((UINTN)ExtEntryList <
           ((UINTN)ExtHeader + ReadUnaligned32(&ExtHeader->ExtHeaderSize))) {
      if (ReadUnaligned16(&ExtEntryList->ExtEntryType) ==
        ...
        *FvUsedSize = ReadUnaligned32(&ExtEntryUsedSize->UsedSize);
        ...
        DEBUG((DEBUG_INFO,
               ...
               FvHeader, *FvUsedSize, *EraseByte));
        return TRUE;
      }
      ExtEntryList =
          (EFI_FIRMWARE_VOLUME_EXT_ENTRY *)((UINT8 *)ExtEntryList +
                                            ReadUnaligned16(
...
CoreProcessFvImageFile(IN EFI_FIRMWARE_VOLUME2_PROTOCOL *Fv,
  ...
  do {
    ...
    BufferSize = 0;
    AlignedBuffer = NULL;
    Status = Fv->ReadSection(Fv, FileName, SectionType, Index, &Buffer,
    ...
    if (!EFI_ERROR(Status)) {
      ...
      if (gSecurity != NULL) {
        ...
        Status = gSecurity->FileAuthenticationState(
            gSecurity, AuthenticationStatus, FvFileDevicePath);
      ...
      // byte of the volume can be aligned on any power-of-two boundary. A
      // weakly aligned volume can not be moved from its initial linked location
      // and maintain its alignment.
      //
      if ((ReadUnaligned32(&FvHeader->Attributes) & EFI_FVB2_WEAK_ALIGNMENT) !=
        ...
        DEBUG((DEBUG_INFO, "%a() FV at 0x%x, FvAlignment required is 0x%x\n",
               __FUNCTION__, FvHeader, FvAlignment));
        ...
        if ((UINTN)FvHeader % FvAlignment != 0) {
          ...
          if (AlignedBuffer == NULL) {
            ...
            if (GetFvUsedSize(FvHeader, &FvUsedSize, &EraseByte)) {
              ...
              SetMem((UINT8 *)AlignedBuffer + FvUsedSize,
                     (UINTN)(BufferSize - FvUsedSize), EraseByte);
            } else {
      ...
      //
      Status = ProduceFVBProtocolOnBuffer((EFI_PHYSICAL_ADDRESS)(UINTN)FvHeader,
                                          (UINT64)BufferSize, FvHandle,
...
VOID EFIAPI CoreFwVolEventProtocolNotify(IN EFI_EVENT Event, IN VOID *Context) {
  ...
  while (TRUE) {
    ...
    Status = CoreLocateHandle(ByRegisterNotify, NULL, mFwVolEventRegistration,
                              &BufferSize, &FvHandle);
    if (EFI_ERROR(Status)) {
      //
    ...
    Status = CoreHandleProtocol(FvHandle, &gEfiFirmwareVolume2ProtocolGuid,
                                (VOID **)&Fv);
    if (EFI_ERROR(Status) || Fv == NULL) {
      //
      // FvHandle must have Firmware Volume2 protocol thus we should never get
      // here.
      //
      ASSERT(FALSE);
      continue;
    ...
    //  EFI_FV_FILETYPE_DXE_CORE is processed to produce a Loaded Image protocol
    //  for the core EFI_FV_FILETYPE_FIRMWARE_VOLUME_IMAGE is processed to
    //  create a Fvb
    //
    for (Index = 0; Index < sizeof(mDxeFileTypes) / sizeof(EFI_FV_FILETYPE);
      ...
      do {
        ...
        GetNextFileStatus =
            Fv->GetNextFile(Fv, &Key, &Type, &NameGuid, &Attributes, &Size);
          ...
          if (Type == EFI_FV_FILETYPE_DXE_CORE) {
            ...
            if (gDxeCoreLoadedImage->FilePath == NULL) {
              if (CompareGuid(&NameGuid, gDxeCoreFileName)) {
                //
                // Maybe One specail Fv cantains only one DXE_CORE module, so
                // its device path must be initialized completely.
                //
            ...
            DepexBuffer = NULL;
            SizeOfBuffer = 0;
            Status = Fv->ReadSection(Fv, &NameGuid, EFI_SECTION_SMM_DEPEX, 0,
                                     &DepexBuffer, &SizeOfBuffer,
                                     &AuthenticationStatus);
            if (!EFI_ERROR(Status)) {
              //
              // If SMM depex section is found, this FV image is invalid to be
              // supported. ASSERT FALSE to report this FV image.
              //
            ...
            //
            // Check if this EFI_FV_FILETYPE_FIRMWARE_VOLUME_IMAGE file has DXE
            // depex section.
            //
            DepexBuffer = NULL;
            SizeOfBuffer = 0;
            Status = Fv->ReadSection(Fv, &NameGuid, EFI_SECTION_DXE_DEPEX, 0,
                                     &DepexBuffer, &SizeOfBuffer,
                                     &AuthenticationStatus);
            if (EFI_ERROR(Status)) {
              //
              // If no depex section, produce a firmware volume block protocol
              // for it so it gets dispatched from.
              //
              CoreProcessFvImageFile(Fv, FvHandle, &NameGuid);
            } else {
              //
              // If depex section is found, this FV image will be dispatched
              // until its depex is evaluated to TRUE.
              //
              FreePool(DepexBuffer);
    ...
    //
    // Read the array of GUIDs from the Apriori file if it is present in the
    // firmware volume
    //
    AprioriFile = NULL;
    Status = Fv->ReadSection(Fv, &gAprioriGuid, EFI_SECTION_RAW, 0,
                             (VOID **)&AprioriFile, &SizeOfBuffer,
                             &AuthenticationStatus);
    if (!EFI_ERROR(Status)) {
    ...
    //
    // Put drivers on Apriori List on the Scheduled queue. The Discovered List
    // includes drivers not in the current FV and these must be skipped since
    // the a priori list is only valid for the FV that it resided in.
    //
...
VOID CoreInitializeDispatcher(VOID) {
  ...
  mFwVolEvent = EfiCreateProtocolNotifyEvent(
      ...
      CoreFwVolEventProtocolNotify, NULL, &mFwVolEventRegistration);
...
loaded because the dependency experessions evaluated to false.
